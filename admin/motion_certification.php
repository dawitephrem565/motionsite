<?php
 ...
 $result=mysqli_query($con,"SELECT * from  certificate ");
 $display_news="";
 while($row=mysqli_fetch_assoc($result))
 {
     
         $id = $row['id'];
         $title= $row["certficate_title"];
        $descr=$row["certificate_desc"];
         $photo=$row["certficate_image"];

       $display_news .=' <div class="col-sm-6">
        <div class="card">
       <img class="card-img-top" src="'.$photo.'" alt=" image cap">
       <div class="card-body">
         <h5 class="card-title">'.$title.'</h5>
         <p class="card-text">'.$descr.'</p>
         <p class="card-text"><small class="text-muted">  </small></p>
         <a href="update_data/update_certificate.php?id='.$id.'&&type=certificate&&src='.$photo.'" class="btn btn-success">Update</a>
         </div>
     </div></div>';
...
if($document)
{
    ...
    $sql =mysqli_query($con,"insert into certificate (certficate_title,certificate_desc,certficate_image,Date) value('$title_upload','$description','$file_path',now())") or die('<script> alert('.mysqli_error().')</script>"');
    header("location: motion_certification.php");
    exit;
}
?>
<!DOCTYPE html>
